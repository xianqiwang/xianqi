...
public class WheelPicker extends android.view.View implements IDebug, IWheelPicker, Runnable {
     ...
     * 滚动状态标识值
     *
     * @see com.nfp.update.WheelPicker.OnWheelChangeListener#onWheelScrollStateChanged(int)
     */
    ...
    private static final String TAG = com.nfp.update.WheelPicker.class.getSimpleName();
     ...
     * 相关监听器
     *
     * @see com.nfp.update.WheelPicker.OnWheelChangeListener, com.nfp.update.WheelPicker.OnItemSelectedListener
     */
    private com.nfp.update.WheelPicker.OnItemSelectedListener mOnItemSelectedListener;
    private com.nfp.update.WheelPicker.OnWheelChangeListener mOnWheelChangeListener;

    private android.graphics.Rect mRectDrawn;
    private android.graphics.Rect mRectIndicatorHead, mRectIndicatorFoot;
    private android.graphics.Rect mRectCurrentItem;

    private android.graphics.Camera mCamera;
    private android.graphics.Matrix mMatrixRotate, mMatrixDepth;
    ...
    public WheelPicker(android.content.Context context, android.util.AttributeSet attrs) {
        super(context, attrs);

        android.content.res.TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.WheelPicker);
        int idData = a.getResourceId(R.styleable.WheelPicker_wheel_data, 0);
        mData = java.util.Arrays.asList(getResources()
        ...
        if (fontPath != null) {
            android.graphics.Typeface typeface = android.graphics.Typeface.createFromAsset(context.getAssets(), fontPath);
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.DONUT) {
            android.view.ViewConfiguration conf = android.view.ViewConfiguration.get(getContext());
            mMinimumVelocity = conf.getScaledMinimumFlingVelocity();
            mMaximumVelocity = conf.getScaledMaximumFlingVelocity();
            mTouchSlop = conf.getScaledTouchSlop();
        }
        mRectDrawn = new android.graphics.Rect();

        mRectIndicatorHead = new android.graphics.Rect();
        mRectIndicatorFoot = new android.graphics.Rect();

        mRectCurrentItem = new android.graphics.Rect();

        mCamera = new android.graphics.Camera();

        mMatrixRotate = new android.graphics.Matrix();
        mMatrixDepth = new android.graphics.Matrix();
    }
    ...
    private void computeTextSize() {
        ...
        } else if (! android.text.TextUtils.isEmpty(mMaxWidthText)) {
        ...
        android.graphics.Paint.FontMetrics metrics = mPaint.getFontMetrics();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int modeWidth = android.view.View.MeasureSpec.getMode(widthMeasureSpec);
        int modeHeight = android.view.View.MeasureSpec.getMode(heightMeasureSpec);

        int sizeWidth = android.view.View.MeasureSpec.getSize(widthMeasureSpec);
        int sizeHeight = android.view.View.MeasureSpec.getSize(heightMeasureSpec);
        ...
        if (isDebug)
            android.util.Log.i(TAG, "Wheel's content size is (" + resultWidth + ":" + resultHeight + ")");
        ...
        if (isDebug)
            android.util.Log.i(TAG, "Wheel's size is (" + resultWidth + ":" + resultHeight + ")");
    ...
    private int measureSize(int mode, int sizeExpect, int sizeActual) {
        int realSize;
        if (mode == android.view.View.MeasureSpec.EXACTLY) {
            realSize = sizeExpect;
        } else {
            realSize = sizeActual;
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH) {
        ...
        if (isDebug)
            android.util.Log.i(TAG, "Wheel's drawn rect size is (" + mRectDrawn.width() + ":" +
    ...
    protected void onDraw(android.graphics.Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            if (mSelectedItemTextColor != -1) {
                ...
                canvas.clipRect(mRectCurrentItem, android.graphics.Region.Op.DIFFERENCE);
    ...
    public boolean onTouchEvent(android.view.MotionEvent event) {
        switch (event.getAction()) {
            case android.view.MotionEvent.ACTION_DOWN:
                ...
                if (null == mTracker)
                    mTracker = android.view.VelocityTracker.obtain();
                else
                    mTracker.clear();
    ...
    public void run() {
        ...
        if (mScroller.isFinished() && !isForceFinishScroll) {
            ...
            if (isDebug)
                android.util.Log.i(TAG, position + ":" + mData.get(position) + ":" + mScrollOffsetY);
    ...
    @Override
    public void setOnItemSelectedListener(com.nfp.update.WheelPicker.OnItemSelectedListener listener) {
    ...
    @Override
    public void setOnWheelChangeListener(com.nfp.update.WheelPicker.OnWheelChangeListener listener) {
    ...
    public interface OnItemSelectedListener {
         ...
         */
        void onItemSelected(com.nfp.update.WheelPicker picker, Object data, int position);
    }
    ...
    public interface OnWheelChangeListener {
         ...
         *              {@link com.nfp.update.WheelPicker#SCROLL_STATE_IDLE}
         *              表示滚动选择器处于静止状态
         *              {@link com.nfp.update.WheelPicker#SCROLL_STATE_DRAGGING}
         *              表示滚动选择器处于拖动状态
         *              {@link com.nfp.update.WheelPicker#SCROLL_STATE_SCROLLING}
         *              表示滚动选择器处于滑动状态
         *              <p>
