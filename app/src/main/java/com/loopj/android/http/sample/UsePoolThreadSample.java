...
public class UsePoolThreadSample extends com.loopj.android.http.sample.GetSample {
    ...
    public com.loopj.android.http.ResponseHandlerInterface getResponseHandler() {
        return new com.loopj.android.http.sample.UsePoolThreadSample.UsePoolThreadResponseHandler();
    }

    private class UsePoolThreadResponseHandler extends com.loopj.android.http.AsyncHttpResponseHandler {
        ...
        public UsePoolThreadResponseHandler() {
            ...
            android.util.Log.d(LOG_TAG, "Bytes will be saved in file: " + destFile.getAbsolutePath());
        ...
        private java.io.File getRandomCacheFile() {
            java.io.File dir = getCacheDir();
            if (dir == null) {
                dir = getFilesDir();
            }

            return new java.io.File(dir, "sample-" + System.currentTimeMillis() + ".bin");
        }

        private void saveBytesOnDisk(java.io.File destination, byte[] bytes) {
            // TODO: Spin your own implementation to save the bytes on disk/SD card.
            if (bytes != null && destination != null) {
                android.util.Log.d(LOG_TAG, "Saved " + bytes.length + " bytes into file: " + destination.getAbsolutePath());
            }
        }
    }
}
